...
filename='SST_monthly_1979_2015_ERAInterim.nc';
...
[pks1,indEN]=findpeaks(PCs(:,1),'minpeakdistance',30);
[pks2,indLN]=findpeaks(-PCs(:,1),'minpeakdistance',30);

indEN=indEN(pks1 >=45); 
indLN=indLN(pks2 >=50);

% one month around each event
indEN=[indEN; indEN-1; indEN+1];
indLN=[indLN; indLN-1; indLN+1];

% plot data in space of the two first two PCs
figure;
plot(PCs(:,1),PCs(:,2),'k.',PCs(indEN,1),PCs(indEN,2),'r.',PCs(indLN,1),PCs(indLN,2),'b.');
xlabel('PC1');
ylabel('PC2');
